...
export const fetchCategories = async (): Promise<Category[]> => {
  ...
  return await database.getAllAsync<Category>('SELECT * FROM categories ORDER BY createdAt DESC;');
};
...
export const fetchVideos = async (): Promise<VideoLink[]> => {
  const database = getDb();
  return await database.getAllAsync<VideoLink>('SELECT * FROM videos ORDER BY createdAt DESC;');
};
...
export const insertVideo = async (video: VideoLink) => {
  ...
  await database.runAsync(
    ...
    video.id, video.url, video.title, video.thumbnailUrl, video.platform, video.categoryId, video.createdAt
  );
};
